 ...
 * @author Z420
 */
public class VerInvitacionesController implements Controller{

    @Override
    public ModelAndView handleRequest(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ModelAndView mv = new ModelAndView("logged/verInvitaciones"); 
        try{
            ...
            if(user != null){
                System.out.println("[ INFO ] VerInvitacionesController.java Cargado");
                List<Intercambio> invitaciones = InvitadosDAO.getMisInvitaciones(email);
                int notificacion = InvitadosDAO.getMisInvitacionesNumero(email);
                mv.addObject("notificacion",notificacion);
                mv.addObject("invitaciones",invitaciones);
                return mv;
            }else{
